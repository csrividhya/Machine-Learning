...
def main():
    ...
    #permutation=[30,2,44,32,31,47,34,40,46,19,10,3,21,50,39,42,24,52,41,35,26,13,22,45,27,17,49,33,23,4,36,14,29,28,48,38,18,53,20,25,6,7,51,1,16,0,15,5,11,9,8,12,43,37]
    X = X[permutation, ]
    Y = Y[permutation, ]
    ...
    for i in range(0,len(l_rate)):
        for j in range(0,len(lamda)):
            ...
            accuracy=crossValidation(X, Y, SgdHinge,maxIter=100, lmda=lmda, learningRate=learningRate, sample=range(20))
    ...
    print("Accuracy (Hinge Loss):\t%s" % crossValidation(X, Y, SgdHinge,maxIter=100, lmda=0.3, learningRate=0.01, sample=range(20)))
    ...
    #print("Accuracy (Hinge Loss):\t%s" % crossValidation(X, Y, SgdHinge,maxIter=100, lmda=0.3 , learningRate=0.01, sample=range(20,X.shape[0])))
